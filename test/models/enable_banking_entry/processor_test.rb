...
class EnableBankingEntry::ProcessorTest < ActiveSupport::TestCase
  ...
  test "prefers merchant line for card payment with location header" do
    name = build_name(
      credit_debit_indicator: "DBIT",
      description: "PAIEMENT CB  1103 59 CROIX",
      remittance_information: [
        "PAIEMENT CB  1103 59 CROIX",
        "ALCEL            CARTE 8496"
      ]
    )

    assert_equal "ALCEL", name
  end
